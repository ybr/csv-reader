...
trait DefaultCsvColumnReaders {
  ...
  implicit val LongCsvReader: CsvColumnReader[Long] = new CsvColumnReader[Long] {
    def read(columnContent: String): CsvResult[Long] = Try(columnContent.toLong) match {
      case Success(int) => CsvSuccess(int)
  ...
  implicit val DoubleCsvReader: CsvColumnReader[Double] = new CsvColumnReader[Double] {
    def read(columnContent: String): CsvResult[Double] = Try(columnContent.toDouble) match {
  ...
  implicit val FloatCsvReader: CsvColumnReader[Float] = new CsvColumnReader[Float] {
    def read(columnContent: String): CsvResult[Float] = Try(columnContent.toFloat) match {
      case Success(int) => CsvSuccess(int)
  ...
  implicit val UUIDCsvReader: CsvColumnReader[UUID] = new CsvColumnReader[UUID] {
    def read(columnContent: String): CsvResult[UUID] = Try(UUID.fromString(columnContent)) match {
...
object DefaultCsvColumnReaders extends DefaultCsvColumnReaders {
  /**
